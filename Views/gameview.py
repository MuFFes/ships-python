...
class GameView(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for x in range(10):
            for y in range(10):
                self.draw_tile(self.my_field_canvas, x, y, "", font=tkutils.get_font("main"))
                self.draw_tile(self.enemy_field_canvas, x, y, "", font=tkutils.get_font("main"))
    ...
    def update_my_field(self, game):
        ...
        for x in range(10):
            for y in range(10):
                self.draw_tile(self.my_field_canvas, x, y, game.my_field.get_state(x, y), font=font)

    def update_enemy_field(self, game):
        font = tkutils.get_font("main")
        for x in range(10):
            for y in range(10):
                self.draw_tile(self.enemy_field_canvas, x, y, game.enemy_field.get_state(x, y), font=font)
    ...
    def clear_ghost_ship(self, x, y, length, orientation):
        for i in range(length):
            if orientation == Orientation.HORIZONTAL:
                self.draw_tile(self.my_field_canvas, x + i, y, "", font=tkutils.get_font("main"))
            else:
                self.draw_tile(self.my_field_canvas, x, y + i, "", font=tkutils.get_font("main"))

    def show_ghost_ship(self, x, y, length, orientation):
        for i in range(length):
            if orientation == Orientation.HORIZONTAL:
                self.draw_tile(self.my_field_canvas, x + i, y, "g", font=tkutils.get_font("main"))
            else:
                self.draw_tile(self.my_field_canvas, x, y + i, "g", font=tkutils.get_font("main"))
